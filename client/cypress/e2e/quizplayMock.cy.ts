...
describe('Quiz Play Component Tests', () => {
  ...
  beforeEach(() => {
    ...
    // Charger la fixture en dehors de l'intercepteur
    cy.fixture('quizQuestions.json').then((data) => {
      questions = data;
    });

    // Intercepter la requête et appliquer la logique personnalisée
    cy.intercept('GET', 'http://localhost:8080/api/question/random/quiz/154', (req) => {
      let excludeIdsParam = req.query['excludeIds'];

      if (typeof excludeIdsParam !== 'string') {
        excludeIdsParam = '';
      }

      const excludeIds = excludeIdsParam ? excludeIdsParam.split(',').map(Number) : [];
      const availableQuestions = questions.filter(q => !excludeIds.includes(q.id));
      const questionToReturn = availableQuestions.length > 0 ? availableQuestions[0] : null;

      // Répondre avec la question filtrée
      req.reply({statusCode: 200, body: questionToReturn});
    }).as('getRandomQuestion');

    cy.visit('/quiz/play/154');
  });

  it('charge la page du quiz (mock) avec les éléments initiaux', () => {
    /*  cy.url().should('include', '/quiz/play/154');*/
    cy.get('.quiz-section').should('be.visible');
    cy.get('.timer-container').should('be.visible');
    cy.get('.question').should('be.visible');
    cy.get('ul.answers').find('li').should('have.length.at.least', 1);
  });
  ...
  it('parcourt toutes les 4 questions mock du quiz', () => {
    ...
    cy.get('.text-orange-600').should('contain', 'Score : 600');

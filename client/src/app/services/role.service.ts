...
export class RoleService {
  ...
  createRole(roles: Role): Observable<Role> {
    return this.http.post<Role>(this.baseUrl, roles, {headers: this.headerUtil.getHeaders()});
  }

  updateRole(roles: Role): Observable<Role> {
    const url = `${this.baseUrl}/${roles.id}`;
    return this.http.put<Role>(url, roles, {headers: this.headerUtil.getHeaders()});
  }
